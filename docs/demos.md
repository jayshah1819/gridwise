---
layout: page
title: Demos
permalink: /demos/
sidebar: demos
---

Interactive visualizations powered by Gridwise WebGPU primitives. Demos run live in the browser and
require a WebGPU-capable browser (Chrome or Edge 113+, or Safari 18+).

## Interactive Demos

### Galaxy Stars

A real-time particle simulation where GPU sort and scan primitives drive the position updates of
thousands of stars. Click anywhere on the canvas to interact with the simulation.

<a href="{{ "/Demos/interactive_demo.html" | relative_url }}" class="doc-btn">Open Demo</a>
<a href="https://github.com/gridwise-webgpu/gridwise/blob/main/Demos/interactive_demo.mjs" target="_blank" class="doc-btn">Source</a>

---
